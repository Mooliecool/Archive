...
*                       _M_IX86 too.
*       06-06-94  SKS   Change if def(_CRTMT) to if def(_CRTMT) || def(_DLL)
...
#if (defined(_CRTMT) || defined(_DLL)) && (!defined(_M_MPPC) && !defined(_M_M68K))
_CRTIMP int * __cdecl _errno(void);
_CRTIMP unsigned long * __cdecl __doserrno(void);
#define errno       (*_errno())
#define _doserrno   (*__doserrno())
#else   /* ndef _CRTMT && ndef _DLL */
